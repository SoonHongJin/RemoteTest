    ...
    public class CMainSystem
    {
        ...
        private bool SaveImage(InspectionInfo _Infos, int _nCurIdx)
        {
            try
            {
                ...
                if (_Infos.sJudge == "OK")
                    sJudgePath = shourPath + "OK Image";
                else
                    sJudgePath = shourPath + "NG Image";
        ...
        /// <summary>
        /// 20250911 SHJ 에이징 테스트 위해 이미지 자동 삭제 위해 메소드 임시 추가 
        /// </summary>
        public void ImageDelete()
        {
            string ImageFolder = DEF_SYSTEM.DEF_FOLDER_PATH_IMAGE;

            string[] folderEntries = Directory.GetDirectories(ImageFolder); // Image

            // 폴더 오래된 순으로 정렬
            folderEntries.OrderBy(ss => new FileInfo(ss).LastWriteTime);

            // 폴더들의 수 체크
            foreach (string FolderPathName in folderEntries)
            {
                try
                {
                    // 폴더들 삭제
                    System.IO.DirectoryInfo dir = new System.IO.DirectoryInfo(FolderPathName);
                    dir.Delete(true);    // false로 할 경우 하위 폴더와 폴더 내의 파일들은 삭제가 안된다.
                }
                catch { }
            }
        }
        ...
        public void SimulTest()
        {
            ...
            for (int j = 0; j < nImgCount; j++)
            {
                ...
                for (int i = 0; i < DEF_SYSTEM.CAM_MAX_COUNT; i++)
                {
                    int nInspIdx = Cameras[i].m_InspIndex;
                    // 20250911 SHJ 현재 레시피에 맞게 필요한 Work 할당 
                    var Work = theRecipe.m_sCurrentEquipment == DEF_SYSTEM.DISPLAY ?
                                Cameras[i].ListInspect[nInspIdx][j] : Cameras[i].InspDeepLearning;
                    Cameras[i].m_nInspMode = DEF_SYSTEM.INSP_NORMAL;
                    Cameras[i].m_bIsInspection = true; // Inspection Mode 
                    ...
                    Cameras[i].SimulationInspectRun(nInspIdx, j);
