...
{
    public partial class CLiveImageControl : UserControl
    {
        ...
        private Rectangle ContrastRet = new Rectangle();
        private Rectangle SharpnessRet = new Rectangle();

        private int m_ImageMode = 0;
        ...
        public CLiveImageControl(MainForm _Mainform)
        {
            Mainform = _Mainform;
        ...
        public void SetCamera(CCameraManager _CameraAction, int mode)   //LYK Mode는 Gray, Color 등으로 구분 될 수 있음
        {
            ...
            if (null != cCameraManager)
            {
                if (mode == 0)   
                {
                    cCameraManager.SetRefreshGrabImage(DEF_SYSTEM.MONO_LIVE_IMAGE_GRAB, SetImage);

                    m_nImageWidth = (int)cCameraManager.m_Camera.m_nImgWidth;   
                    m_nImageHeight = (int)cCameraManager.m_Camera.m_nImgHeight;
                }
        ...
        public void ZoomFit()
        {
            ...
            // 250530 SHJ 임시 주석 
            /*
            if (Id == DEF_SYSTEM.CAM_ONE || Id == DEF_SYSTEM.CAM_THREE )
            {
                float zoomX = ((float)(View.Width) / (float)m_nImageWidth);
                float zoomY = ((float)(View.Height) / (float)m_nImageHeight);
                bool horizontal = (zoomX > zoomY) ? true : false;
                ...
                imgx = 0;
                imgy = 0;
                //imgx = horizontal ? View.Width + 400 : View.Width / 10;
                //imgy = horizontal ? 0 : (View.Height - zoom * m_nImageHeight) / (2 * zoom);
                Refresh();
            }
            else if (Id == DEF_SYSTEM.CAM_TWO || Id == DEF_SYSTEM.CAM_FOUR )
            {
                float zoomX = ((float)(View.Width) / (float)m_nImageWidth);
                float zoomY = ((float)(View.Height) / (float)m_nImageHeight);
                bool horizontal = (zoomX > zoomY) ? true : false;
        ...
        private void OnPaint(object sender, PaintEventArgs e)
        {
            ...
            string LighrValue = string.Empty;
