...
{
    public partial class CHistoryImageControl : UserControl
    {
        ...
        private FormResultHistory History = null;
        ...
        public CHistoryImageControl(MainForm _Mainform)
        {
            Mainform = _Mainform;
        ...
        public void SetCamera(CCameraManager _CameraAction, FormResultHistory _History)
        {
            ...
            {
                cCameraManager.SetRefreshHistoryImage(SetImage);    //240123 LYK ResfreshHistoryImage
        ...
        private void OnPaint(object sender, PaintEventArgs e)
        {
            ...
            if (Image != null)
            {
                //250207 KCH 각 검사데이터에 따라서 FormResultHistory에 이미지 Paint
                if (theMainSystem.mainForm.ResultHistory.InspectionDataForm.Visible && theMainSystem.mainForm.ResultHistory.InspectionDataForm.bInspectionImagefound)
                {
                    if(theMainSystem.mainForm.ResultHistory.InspectionDataForm.SellClick)
                        History.DoPaintSelectInspectionData(graphic, Id);
                    else
                        History.DoPaintInspectionData(graphic, Id);
                }
            }   
            else
            {   
                //History.bFirstCheck = false;
            }
