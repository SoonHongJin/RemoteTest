...
namespace Core.Utility
{
    public partial class CSimulImageControl : UserControl
    {
        ...
        private void OnPaint(object sender, PaintEventArgs e)
        {
            ...
            if (Image != null)
            {
                // 20250916 SHJ 시뮬레이션 디스플레이 표시는 Main Insp 검사 결과 표시하는 것 과 동일 해서 같은 메소드 호출 하는 방식으로 우선 사용
                if (Id <= DEF_SYSTEM.CAM_FOUR)
                    cCameraManager.DoSimulDefectCurrentPaint(graphic, Id);
