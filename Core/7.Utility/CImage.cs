    ...
    public class CImage
    {
        ...
        public void ImgRead(string sPath, int _ImgIdx, int _StartPosX = 0)
        {
            try
            {
                ...
                {
                    if(m_sCrruentEqupiment == DEF_SYSTEM.ELECTRONIC)
                    {
        ...
        public bool Copy(int nWidth, int nHeight, int nPixelFormat, IntPtr pImage, int _nStartposX = 0, int _nStandardWidth = 0, int _ImgCount = 0)
        {
            ...
            if (m_nLicense == CDefineDLL.COG)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < m_nSliceCount; i++)
                    {
                        int sliceX = _nStartposX + i * (_nStandardWidth - m_nSliceWidthOffset); //20250821 shj 임시 Slice Offset
        ...
        public bool Merge(int nWidth, int nHeight, int nPixelFormat, IntPtr pImage, int nImageCount, bool bReduced = false)
        {
            ...
            if (nImageCount == m_nMergeCount - 1)
            {
                ...
                if (m_sCrruentEqupiment == DEF_SYSTEM.DISPLAY)
                {
                    for (int i = 0; i < m_nSliceCount; i++)
                    {
                        GrayCogRoot[i].Initialize(m_nSliceWidth, m_nHeight * m_nMergeCount, pSliceData[i], m_nSliceStride, null);
        ...
        public void Crop(CImage _Image, int nPixelFormat, int nWidth, int nHeight, int Xpos, int Ypos, int nSliceIndex = 0)
        {
            lock (CropLock)
            {
                if (Xpos < 0)
                ...
                // 250911 SHJ Display 같은 경우 Slice 된 이미지에서 Crop 처리 
                if(m_sCrruentEqupiment == DEF_SYSTEM.DISPLAY)
                {
                    if (Xpos + nWidth > _Image.m_nSliceWidth)
                        nWidth = _Image.m_nSliceWidth - Xpos;
                    if (Ypos + nHeight > _Image.m_nSliceHeight)
                        nHeight = _Image.m_nHeight - Ypos;
                }
                else 
                {
                    if (Xpos + nWidth > _Image.m_nWidth)
                        Xpos = _Image.m_nWidth - nWidth;

                    if (Ypos + nHeight > _Image.m_nHeight)
                        Ypos = _Image.m_nHeight - nHeight;
                }

                // 250911 SHJ Display or Electronic 으로 구분 하여 사용 
                int srcWidth = m_sCrruentEqupiment == DEF_SYSTEM.DISPLAY ? _Image.m_nSliceWidth : _Image.m_nWidth;
                int srcHeight = m_sCrruentEqupiment == DEF_SYSTEM.DISPLAY ? _Image.m_nSliceHeight : _Image.m_nHeight;
                int stride = m_sCrruentEqupiment == DEF_SYSTEM.DISPLAY ? _Image.m_nSliceStride : _Image.m_nStride;
                ...
                for (int i = 0; i < nHeight; i++)
                {
                    ...
                    // 250911 SHJ Display or Electronic 으로 구분 하여 사용 
                    IntPtr ptr = m_sCrruentEqupiment == DEF_SYSTEM.DISPLAY ? _Image.pSliceData[nSliceIndex] : _Image.pData;
