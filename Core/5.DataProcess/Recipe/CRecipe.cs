...
namespace Core.DataProcess
{
    ...
    {
        public int m_nCellRow { get; set; } = 0;            //250909 SHJ 자재 정보 Cell 가로 방향 배열 갯수
        public int m_nCellColumn { get; set; } = 0;         //250909 SHJ 자재 정보 Cell 세로 방향 배열 갯수 
        public int m_nGroupCount { get; set; } = 0;         //250915 SHJ 자재 Cell 배열 그룹 갯수
        public double m_dActiveWidth { get; set; } = 0;      //250909 SHJ 자재 정보 Cell Active Width
        public double m_dActiveHeight { get; set; } = 0;     //250909 SHJ 자재 정보 Cell Active Height
        ...
        public double m_dDumylength { get; set; } = 0;       // 250915 SHJ 디스플레이 기판 자재 정보중 그룹간 사이 dumy 길이 
    }
    ...
    public class CRecipe
    {
        ...
        public int m_nAutoDeleteMode = 1; // 20250917 SHJ Auto Delete Mode 0: Date Delete, 1: Drive Delete 
        public int m_nDateLimit;
        public double m_dDriveLimit = 45;
        ...
        public int SliceImageCount { get; set; } = 18;
        ...
        public bool DataLoad(string _sModleName)
        {
            ...
            try
            {
                ...
                SliceImageCount = m_sCurrentEquipment == DEF_SYSTEM.DISPLAY ? 18 : 4;

                JObject jsonMaterial = (JObject)jsonSetup["Material"] != null ? (JObject)jsonSetup["Material"] : new JObject();
                MaterialInfo.m_nCellRow = CheckState(jsonMaterial, "m_nCellRow") ? 6 : int.Parse(jsonMaterial["m_nCellRow"].ToString());
                MaterialInfo.m_nCellColumn = CheckState(jsonMaterial, "m_nCellColumn") ? 5 : int.Parse(jsonMaterial["m_nCellColumn"].ToString());
                MaterialInfo.m_nGroupCount = CheckState(jsonMaterial, "m_nGroupCount") ? 3 : int.Parse(jsonMaterial["m_nGroupCount"].ToString());
                MaterialInfo.m_dActiveWidth = CheckState(jsonMaterial, "m_dActiveWidth") ? 5300 : double.Parse(jsonMaterial["m_dActiveWidth"].ToString());
                MaterialInfo.m_dActiveHeight = CheckState(jsonMaterial, "m_dActiveHeight") ? 11400.0 : double.Parse(jsonMaterial["m_dActiveHeight"].ToString());
                MaterialInfo.m_dCellPadHeight = CheckState(jsonMaterial, "m_dCellPadHeight") ? 960.0 : double.Parse(jsonMaterial["m_dCellPadHeight"].ToString());
                MaterialInfo.m_dHolePosX = CheckState(jsonMaterial, "m_dHolePosX") ? 2620.0 : double.Parse(jsonMaterial["m_dHolePosX"].ToString());
                MaterialInfo.m_dHolePosY = CheckState(jsonMaterial, "m_dHolePosY") ? 255.0 : double.Parse(jsonMaterial["m_dHolePosY"].ToString());
                MaterialInfo.m_dHoleRadius = CheckState(jsonMaterial, "m_dHoleRadius") ? 150.0 : double.Parse(jsonMaterial["m_dHoleRadius"].ToString());
                MaterialInfo.m_dDumylength = CheckState(jsonMaterial, "m_dDumylength") ? 820.0 : double.Parse(jsonMaterial["m_dDumylength"].ToString());
        ...
        public void DataSave(string _sModleName)
        {
            ...
            try
            {
                ...
                jsonMaterial.Add("m_nCellRow", MaterialInfo.m_nCellRow);
                jsonMaterial.Add("m_nCellColumn", MaterialInfo.m_nCellColumn);
                jsonMaterial.Add("m_nGroupCount", MaterialInfo.m_nGroupCount);
                jsonMaterial.Add("m_dActiveWidth", MaterialInfo.m_dActiveWidth);
                jsonMaterial.Add("m_dActiveHeight", MaterialInfo.m_dActiveHeight);
                ...
                jsonMaterial.Add("m_dDumylength", MaterialInfo.m_dDumylength);
