...
namespace Core.UI
{
    public partial class FormInsCalibrationCheckerBoard : Form
    {

        private MainForm MainForm;
        private FormCalibrationScreen CalibMainForm;
        private List<IInsTool> m_TeachingToolList = new List<IInsTool>();   //Teaching InsTool List
        ...
        private InsToolBlock _ToolBlock = null;     //InsToolBlock
        ...
        private InsRecordDisplayControl[] CalibImageDisplay = new InsRecordDisplayControl[5];
        ...
        public FormInsCalibrationCheckerBoard(MainForm _MainForm, FormCalibrationScreen _CalibMainForm, CLogger _logger)
        {
            InitializeComponent();
        ...
        private void RecipeSave()
        {
            try
            {
                theRecipe.m_dCheckerBoardTitleSize.X = (float)Math.Round(float.Parse(tb_TileSizeX.Text), 3);
                theRecipe.m_dCheckerBoardTitleSize.Y = (float)Math.Round(float.Parse(tb_TileSizeY.Text), 3);
            }
            catch (Exception ex)
            {
        ...
        {
            _ToolBlock = ((InsToolBlock)theRecipe.InsCalibrationToolBlock.Tools[m_nSelectCam]);
                                                        //임시 주석
            ToolBlockEdit ToolEdit = new ToolBlockEdit(/*ToolBlockEditTitle_Offset +*/ m_nInspectSelected, null,_ToolBlock); //240703 NIS 실행한 TeachMode에 따라서 ToolBlockEdit 타이틀 수정
        ...
        private void btn_Run_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < DEF_SYSTEM.CAM_MAX_COUNT; i++)
            {
                ToolBlockRun(i, theRecipe.InsCalibrationToolBlock.Tools[i] as InsToolBlock, m_nInspectSelected);
            }
            m_nToolSelected = (int)lib_ToolList.SelectedIndex;
            IInsTool SelectTool = m_TeachingToolList[m_nToolSelected];

            if (SelectTool.Name.Contains("CalibCheckerboardTool"))// Tool 이름으로 구분하여 Parameter 표시
            {
                InsCalibCheckerboardTool CalibTool = SelectTool as InsCalibCheckerboardTool;
        ...
        private void Run_ApplyAndSave()
        {
            try
            {
                IInsTool tool = m_TeachingToolList[m_nToolSelected];


                if (tool.Name.Contains("InsCalibCheckerboardTool")) // Tool 이름으로 구분하여 Parameter Apply
                {
                    InsCalibCheckerboardTool CalibTool = tool as InsCalibCheckerboardTool;
        ...
        private void ToolListShow()   //240720 NIS Set ToolList
        {
            ...
            if (theRecipe.InsCalibrationToolBlock == null)
                return;

            for (int i = 0; i < ((InsToolBlock)theRecipe.InsCalibrationToolBlock.Tools[m_nSelectCam]).Tools.Count; i++)
            {
                IInsTool tool = ((InsToolBlock)theRecipe.InsCalibrationToolBlock.Tools[m_nSelectCam]).Tools[i];
                string sTemp = tool.Name;

                if (sTemp.Contains("InsFindLine"))
                {
                    lib_ToolList.Font = font;
                    lib_ToolList.Items.Add(sTemp);
                    m_TeachingToolList.Add(tool);
                }
                else if (sTemp.Contains("CalibCheckerboardTool"))
                {
                    lib_ToolList.Font = font;
                    lib_ToolList.Items.Add(sTemp);
                    m_TeachingToolList.Add(tool);
        ...
        private void CalibToolParameterApply(InsCalibCheckerboardTool CalibTool)
        {
            try
            {
        ...
        {
            //임시 주석
            //_toolBlock.Inputs[0].Value = theMainSystem.Cameras[_nCamNum].BeltImages[0][m_nImageType].InsImage;
            _toolBlock.Run();
        }
        private void SetTextCalibToolData(InsCalibCheckerboardTool CalibTool)
        {
        ...
        private void CalibCheckerBoardToolShow(InsCalibCheckerboardTool CalibTool)
        {
            InsRecord CurrRecord = new InsRecord();
            CurrRecord.ContentType = typeof(IInsImage);
            CurrRecord.Content = CalibTool.InputImage;
        ...
        private void TeachInspectionRun()   //240709 NIS Cam 변경 시 마다 Inspection Display
        {
            ...
            {
                /*  임시 주석
                InsRecord MainRecord = new InsRecord("Main", theMainSystem.Cameras[i].BeltImages[0][m_nImageType].GetType(), InsRecordUsageConstants.Result, false, theMainSystem.Cameras[i].BeltImages[0][m_nImageType].InsImage, "Main");

                CalibImageDisplay[i].Image = theMainSystem.Cameras[i].BeltImages[0][m_nImageType].InsImage;
                CalibImageDisplay[i].Record = MainRecord;
                */
                CalibImageDisplay[i].FitImage();
            }
        }
        ...
        private void FormLocate()
        {
            ...
            for (int i = 0; i < DEF_SYSTEM.CAM_MAX_COUNT; i++)
            {
                CalibImageDisplay[i] = new InsRecordDisplayControl
                {
        ...
        {
            //for (int i = 0; i < theRecipe.m_nMaxCamCount; i++) //241015 NIS Screen이 만들어진 후 수행가능한 동작
            //{
        ...
        {
            //theMainSystem.DoBeltCalibGrabStart(DEF_SYSTEM.BELT_CALIB);

            InsCalibCheckerboardTool CalibTool = m_TeachingToolList[lib_ToolList.SelectedIndex] as InsCalibCheckerboardTool;
            ...
            using (OpenFileDialog fileDialog = new OpenFileDialog())
            {
                ...
                if (dr == DialogResult.OK)
                {
                    try
                    {
                        ...
                        InsCalibCheckerboardTool CalibTool = ((InsToolBlock)theRecipe.InsCalibrationToolBlock.Tools[m_nSelectCam]).Tools[0] as InsCalibCheckerboardTool;
                        CalibTool.InputImage = new InsImage8Grey(new Bitmap(m_sArrCalibImagesName[0]));
        ...
        private void Grab_CalibImage(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    for (int i = 0; i < DEF_SYSTEM.CAM_MAX_COUNT; i++)
                    {
                        InsCalibCheckerboardTool CalibTool = ((InsToolBlock)theRecipe.InsCalibrationToolBlock.Tools[i]).Tools[0] as InsCalibCheckerboardTool;
                        CalibTool.Calibration.CalibrationImage = CalibTool.InputImage;
                        CalibTool.Calibration.Calibrate();
                        CalibTool.Run();
                        if (i == m_nSelectCam)
        ...
        private void Save_CalibImage(object sender, EventArgs e)
        {
            ...
            {
                InsCalibCheckerboardTool CalibTool = ((InsToolBlock)theRecipe.InsCalibrationToolBlock.Tools[i]).Tools[0] as InsCalibCheckerboardTool;
                InsImage8Grey calibImage = CalibTool.Calibration.CalibrationImage as InsImage8Grey;
                cCalibImage.InsImage = calibImage;
