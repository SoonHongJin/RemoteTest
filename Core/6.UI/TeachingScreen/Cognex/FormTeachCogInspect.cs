        ...
        private void CreatLabeling(CogImage8Grey _Image, CogBlobTool _BlobTool)
        {
            ...
            if (_Image != null)
            {
                try
                {
                    ...
                    CogImage8Grey InspImage = new CogImage8Grey();
                    int InpIdx = theMainSystem.Cameras[m_nSelectCam].m_InspIndex;
                    ...
                    List<CogBlobResult> OutputResult = InspToolBlock.Outputs[1].Value as List<CogBlobResult>;
                    int Count = theMainSystem.ProductInfo.TeachDefectManager[theMainSystem.Cameras[m_nSelectCam].m_InspIndex][ImageIndex].Count;

                    // 250909 SHJ 검사 완료된 위치 이미지 항목에 표시 
                    for (int i = 0; i < Count; i++)
                    {
                        double CenX = (float)theMainSystem.ProductInfo.TeachDefectManager[InpIdx][ImageIndex][i].InspResult.DefectPos.X; // Math.Round(OutputResult[i].CenterOfMassX, 3);
                        double CenY = (float)theMainSystem.ProductInfo.TeachDefectManager[InpIdx][ImageIndex][i].InspResult.DefectPos.Y; //Math.Round(OutputResult[i].CenterOfMassY, 3);
                        double Width = theMainSystem.ProductInfo.TeachDefectManager[InpIdx][ImageIndex][i].InspResult.Width;//Math.Round(OutputResult[i].GetMeasure(CogBlobMeasureConstants.BoundingBoxPixelAlignedNoExcludeWidth), 3);
                        double Height = theMainSystem.ProductInfo.TeachDefectManager[InpIdx][ImageIndex][i].InspResult.Height;//Math.Round(OutputResult[i].GetMeasure(CogBlobMeasureConstants.BoundingBoxPixelAlignedNoExcludeHeight), 3);
