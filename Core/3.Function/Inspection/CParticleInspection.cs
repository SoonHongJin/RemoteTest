    ...
    public class CParticleInspection : CWork, IInspection
    {
        ...
        public void FindDefect(int _nImageCnt, int _nInspecIdx, CImage _Image)
        {
            try
            {
                ...
                m_InspectToolBlock.Inputs[1].Value = theRecipe.MaterialInfo.m_nCellColumn; //5;      // 20250908 SHJ Cell 세로 갯수
                m_InspectToolBlock.Inputs[2].Value = theRecipe.MaterialInfo.m_dActiveHeight; //10560.0;   // 20250908 SHJ Cell 엑티브 Height                                  
                ...
                else
                {
                    ...
                    int ImageOffsetX = _Image.m_nSliceStartPosX + (_Image.m_nStandardWidth * _nImageCnt) + _Image.m_nSliceJump[_nImageCnt];
                    ...
                    for (int i = 0; i < m_BlobResults.Count; i++)
                    {
                        ...
                        defect.InspResult.DefectPos = new PointF((float)Math.Round(m_BlobResults[i].CenterOfMassX + (InspMode != DEF_SYSTEM.INSP_TEACH ? ImageOffsetX : 0), 3), (float)Math.Round(m_BlobResults[i].CenterOfMassY, 3)); // 240430 SHJ 현재 이미지 카운트에 맞게 Offset 처리 하여 위치 저장 
