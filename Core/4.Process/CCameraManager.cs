...
namespace Core.Process
{
    public class CCameraManager
    {
        ...
        public void Initialize(bool bSimulationMode)
        {
                ...
                for (int i = 0; i < theRecipe.MergeImageCount; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (i == 0)
                        {
                            ...
                            cImage.Allocate((int)m_Camera.m_nImgWidth, (int)m_Camera.m_nImgHeight, CImage.PIXEL8, theRecipe.MergeImageCount, true, 5, 12460, 5900, 800);
                        }
            ...
            {
                string ImagePath = theRecipe.m_sCurrentEquipment == DEF_SYSTEM.ELECTRONIC ? $"D:\\CIS Image_Electronic\\851609Test_Cam1_{j}.bmp" : $"D:\\CIS Image_Display\\레시피3_이물\\{j}.bmp";
                SimulationImgRead(m_InspIndex, j, ImagePath);
            }
        ...
        private void ShowImageDisplay()
        {
            try
            {
                if (m_bIsInspection)
                {
                    ...
                    {
                        mainForm.TeachingInspection.Get_DeepLearningTeachingForm().Frames[this.m_nCamNumber].Invoke(new System.Windows.Forms.MethodInvoker(delegate ()
                        {
                            RefreshTeachImage?.Invoke(ListImages[m_nCurIndex][m_nImageCnt]);
                        }));
                    }
        ...
        private void DeepLearningRun(int InspIdx, int ImgCount)
        {
            ...
            if (theRecipe.m_sCurrentEquipment.Contains(DEF_SYSTEM.DISPLAY))
            {
                ...
                if (WaitHandle.WaitAll(ImageList.ToArray(), int.MaxValue))
                {
                    ...
                    Logger.Logging(DEF_SYSTEM.LOGTYPE[(int)DEF_SYSTEM.LOGTYPE_ENUM.SystemLog], $"DeepLearning Inspect Start");
                    // 디스플레이 딥러닝 실행
        ...
        {
            // 250530 SHJ Insp Mode Normal 일 경우 메인 진입 하도록 추가
            if (m_nInspMode == DEF_SYSTEM.INSP_NORMAL)
            {
        ...
        {
            //int nCount = theMainSystem.ProductInfo.DefectManager[this.m_nCamNumber][ImgIdx].Count;
            //
            //if (nCount > 0)
            //{
            //    for (int i = 0; i < nCount; i++)
            //        theMainSystem.ProductInfo.DefectManager[this.m_nCamNumber][ImgIdx][i].Dispose();
            //}
            //
            //theMainSystem.ProductInfo.DefectManager[this.m_nCamNumber][ImgIdx].Clear();

            //ListInspect[RunIdx][ImgIdx].WorkComplete(theMainSystem.ProductInfo.DefectManager[m_nCamNumber][ImgIdx], null, ImgIdx, RunIdx);  //220503 LYK 옵저버 컬렉션을 이용한 Defect Data 취합
            ...
            if(m_nInspMode == DEF_SYSTEM.INSP_NORMAL)
            {
                if (theRecipe.m_sCurrentEquipment == DEF_SYSTEM.DISPLAY) //디스플레이 룰베이스 -> 딥러닝 순서 , 룰베이스 종료 후 딥러닝 실행 
                {
                    if (DEF_SYSTEM.LICENSES_DEEP_KEY == (int)DeepLicense.SAIGE)
                    {
                        ...
                        if (ImgIdx == 0)
                        {
                            // 250910 SHJ 딥러닝 프로세스 실행 -> 구조는 변경 될 수 있음 작동 확인 위해 임시 사용
        ...
        public void SimulationInspectRun(int CurIndex,int ImageCnt)
        {
            ...
            if (theRecipe.m_sCurrentEquipment == DEF_SYSTEM.DISPLAY)
                Task.Run(() => InspectionRun(CurIndex, ImageCnt, DEF_SYSTEM.INSP_NORMAL));
            else if (theRecipe.m_sCurrentEquipment == DEF_SYSTEM.ELECTRONIC)
        ...
        {
            m_InspIndex = CurIndex;
            m_nImageCnt = _ImgCount;
            ...
            using (Bitmap bmp = new Bitmap(_ImagePath))
            {
                ...
                if(theRecipe.m_sCurrentEquipment == DEF_SYSTEM.ELECTRONIC)
                {
                    ListImages[m_InspIndex][m_nImageCnt].Copy(bmpData.Width, bmpData.Height, 1, bmpData.Scan0);  
                    ListImages[m_InspIndex][0].Merge(bmpData.Width, bmpData.Height, 1, bmpData.Scan0, _ImgCount, true);
                }
                else
                {
                    if (m_nImageCnt != 0)
                    ...
                    ListImages[m_InspIndex][0].Copy(bmpData.Width, bmpData.Height, 1, bmpData.Scan0, 12460, 5900, _ImgCount);
                    ListImages[m_InspIndex][0].Merge(bmpData.Width, bmpData.Height, 1, bmpData.Scan0, _ImgCount, true);
                }    
        ...
        public void ToolBlockRefresh()
        {
            ...
            for(int i = 0; i < DEF_SYSTEM.INSP_CNT; i ++)
            {
                for (int j = 0; j < theRecipe.MergeImageCount; i++)
                {
